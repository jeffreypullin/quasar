...
Interaction testing can be performed for bulk/pseudobulk data with the linear mixed model (`lmm`), linear model (`lm`), Negative Binomial GLM (`nb_glm`), Poisson GLMM (`p_glmm`) and Negative Binomial GLMM (`nb_glmm`) models, and for single-cell data with the Poisson GLMM (`p_glmm_sc`).
